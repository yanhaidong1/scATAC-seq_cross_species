...
loadData           <- function(meta, 
    ...
    marker.info <- read.delim(markers,header=T)
    ...
    ##updating 102121
    ##filter cluster with a small number of cells
    data_meta <- as.data.frame(table(b[,target_cluster]))
    #data_meta <- as.data.frame(table(b$Cluster))
    rmcluster <- data_meta[data_meta$Freq<2,1]
    #flt_b <- b[b[,target_cluster] != rmcluster,]
    flt_b <- b[!b[,target_cluster] %in% rmcluster,]
    #flt_b <- b[b$Cluster != rmcluster,]
    ##filter based on the cellname of flt_b
    activity <- activity[,rownames(flt_b)]
    h.pcs <- h.pcs[rownames(flt_b),]
    ...
    # output
    return(list(h.pcs=h.pcs, b=flt_b, activity=activity, marker.info=marker.info))
...
validateCluster    <- function(df,target_cluster){
    if(! "Cluster" %in% colnames(df)){
        ...
        ##first consider the LouvainClusters_new
        if (target_cluster == 'LouvainClusters_new') {
          if ('LouvainClusters_new' %in% colnames(df)){
            df$Cluster <- paste0("cluster", as.numeric(as.factor(df$LouvainClusters_new)))
            return(df)
          }
        }else{
          ...
          if (target_cluster == 'LouvainClusters_afthm') {
            if ('LouvainClusters_afthm' %in% colnames(df)){
              ...
              if (target_cluster == 'LouvainClusters_sub') {
                if ('LouvainClusters_sub' %in% colnames(df)){
                  df$Cluster <- paste0("cluster", as.character(as.factor(df$LouvainClusters_sub)))
                  return(df)
                }
            
              }else{
                
                if (target_cluster == 'Final_annotation_TCP') {
                  if ('Final_annotation_TCP' %in% colnames(df)){
                    df$Cluster <- paste0("CT", as.character(as.factor(df$Final_annotation_TCP)))
                    return(df)
                  }
...
clusterAves        <- function(output_dir,df, 
    ...
    # plot
    pdf(paste0(output_dir,'/',output,"_markers_heatmap.pdf"), width=10, height=12)
    heatmap.2(c.means, trace='none', scale='row', Colv=F, Rowv=F, dendrogram='none',
...
plot.act.scores_eachCT    <- function(marker_opt_dir,df, 
  ...
  for (i in 1:length(unique_type_list)) {
    ...
    png(file=paste0(marker_opt_dir,'/',target_celltype,'.pdf'), width=12, height=ratio*12, units="in", res=500, type="cairo")
...
plot.new.markers   <- function(output_dir,df=NULL, 
    ...
    write.csv(all_top.genes,paste0(output_dir,'/',outname,"opt_rank_difs.csv"))
